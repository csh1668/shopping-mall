    ...
    <div className={`space-y-6 ${className}`}>
      ...
      {loading ? (
        <div className={`grid gap-4 ${
          ...
          {[...Array(8)].map((_, i) => (
            <Card key={i} className={viewMode === 'list' ? 'flex' : ''}>
              <CardContent className={viewMode === 'list' ? 'flex gap-4 p-4' : 'p-4'}>
        ...
        <div className={`grid gap-4 ${
          ...
          {results.map((result) => (
            <Card key={result.id} className={viewMode === 'list' ? 'flex' : 'group hover:shadow-lg transition-shadow'}>
              <CardContent className={viewMode === 'list' ? 'flex gap-4 p-4' : 'p-4'}>
                ...
                <div className="flex-1 space-y-2">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <h3 className="font-medium line-clamp-2">{result.name}</h3>
                      <p className="text-sm text-muted-foreground">{result.brand}</p>
                    </div>
                    {result.onSale && (
                      <Badge variant="destructive" className="ml-2">
                        {calculateDiscount(result.price, result.salePrice!)}% 할인
                      </Badge>
                    )}
                  </div>
              ...
              <CardFooter className={viewMode === 'list' ? 'flex-shrink-0' : ''}>
                <Button 
                  className="w-full" 
