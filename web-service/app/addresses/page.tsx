...
export default function AddressesPage() {
  ...
  const handleEditAddress = (address: Address) => {
    // 기본 배송지로 설정하는 경우 다른 주소들의 기본 설정 해제
    if (address.isDefault) {
      setAddresses((prev) => prev.map((addr) => (addr.id === address.id ? address : { ...addr, isDefault: false })))
    } else {
          ...
          <div className="container px-4 py-8">
            <div className="max-w-2xl mx-auto">
              <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                  <h1 className="text-2xl font-bold">{editingAddress ? "배송지 수정" : "새 배송지 추가"}</h1>
                </div>
              </div>
            ...
            {addresses.length === 0 ? (
              <Card>
                <CardContent className="p-12 text-center">
                  ...
                  <p className="text-muted-foreground mb-6">새 배송지를 추가해보세요</p>
                  <Button onClick={() => setShowForm(true)}>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        {address.detailAddress && <p className="text-sm">{address.detailAddress}</p>}
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button variant="outline" size="icon" onClick={() => handleEditClick(address)}>
        ...
        <AlertDialog open={!!deleteTarget} onOpenChange={() => setDeleteTarget(null)}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>삭제된 배송지는 복구할 수 없습니다.</AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel>취소</AlertDialogCancel>
              <AlertDialogAction
                onClick={() => deleteTarget && handleDeleteAddress(deleteTarget)}
